...
const EditProfileScreen: React.FC<Props> = ({ user, onBack, onUpdate }) => {
  ...
  const calculatePromotionRank = (data: Member) => {
  ...
  const handleSave = () => {
    if (formData.position === "Huynh trưởng" && !formData.isOrdained && (formData.rank === "Định" || formData.rank === "Lực")) {
      setError("Không thể lưu. Vui lòng chọn lại bậc học hoặc thọ cấp.");
      return;
    }
    onUpdate(formData);
  };
      ...
      <KeyboardAvoidingView behavior={Constants.IS_IOS ? 'padding' : undefined} style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.formContainer}>
            <EditField label="Họ tên" value={formData.fullName} onChange={(v) => handleChange('fullName', v)} />
            <EditField label="Pháp danh" value={formData.dharmaName} onChange={(v) => handleChange('dharmaName', v)} />

            <SelectField label="Giới tính" value={formData.gender} options={GENDERS} onChange={(v) => handleChange('gender', v)} />
              ...
              <View style={styles.ordainedSection}>
                ...
                {formData.isOrdained && (
                  <View style={styles.ordainedDetails}>
                    <EditField label="Ngày thọ cấp" value={formData.ordinationDate || ''} onChange={(v) => handleChange('ordinationDate', v)} />
            ...
            <EditField label="Email" value={formData.email} onChange={(v) => handleChange('email', v)} />
            <EditField label="Số điện thoại" value={formData.phone} onChange={(v) => handleChange('phone', v)} />
            <EditField label="Ngày bắt đầu sinh hoạt" value={formData.joinDate} onChange={(v) => handleChange('joinDate', v)} />
          </View>
